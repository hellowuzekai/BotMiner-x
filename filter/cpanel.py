...
def pcap_filter(path):
    ...
    if os.path.exists(new_path):
        if raw_input('[filter] Filtered pcap file already exists: {}, [R]emove or [U]se it? '.format(new_path)) in ['U',
            ...
            os.remove(new_path)
    # command = 'tshark -r {} -Y "not ipv6 and (tcp or udp) and not ((ip.src==127.0.0.0/8 or ip.src==192.168.0.0/16 or ip.src==10.0.0.0/8 or ip.src==172.16.0.0/12) and (ip.dst==127.0.0.0/8 or ip.dst==192.168.0.0/16 or ip.dst==10.0.0.0/8 or ip.dst==172.16.0.0/12))" -w {} -F pcap'.format(path, new_path)
